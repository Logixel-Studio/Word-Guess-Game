...
export default function ExpenseTypeForm({ open, onClose, editing }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) => editing ? base44.entities.ExpenseType.update(editing.id, data) : base44.entities.ExpenseType.create(data),
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>{editing ? 'Edit Expense Type' : 'Create Expense Type'}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
          <div><Label>Name *</Label><Input {...register('name', { required: true })} placeholder="Expense type name" /></div>
