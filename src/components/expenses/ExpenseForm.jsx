...
export default function ExpenseForm({ open, onClose, editing, expenseTypes }) {
  ...
  useEffect(() => {
    if (editing) {
      reset({
        expense_type_id: editing.expense_type_id, qty: editing.qty, unit_price: editing.unit_price,
        description: editing.description, payment_status: editing.payment_status || 'unpaid',
        paid_amount: editing.paid_amount || 0, due_date: editing.due_date || '',
      });
    } else {
      reset({ expense_type_id: '', qty: '', unit_price: '', description: '', payment_status: 'unpaid', paid_amount: 0, due_date: '' });
    }
  }, [editing, open, reset]);
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
          <div>
            ...
            <Select value={watch('expense_type_id') || ''} onValueChange={v => setValue('expense_type_id', v)}>
              <SelectTrigger><SelectValue placeholder="Select expense type" /></SelectTrigger>
              <SelectContent>{expenseTypes.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
