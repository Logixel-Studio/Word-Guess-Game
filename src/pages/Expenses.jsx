...
export default function Expenses() {
  ...
  const { data: expenses = [], isLoading } = useQuery({ queryKey: ['expenses'], queryFn: () => base44.entities.Expense.list() });
  const { data: expenseTypes = [], isLoading: loadingTypes } = useQuery({ queryKey: ['expenseTypes'], queryFn: () => base44.entities.ExpenseType.list() });
    ...
    { key: 'created_date', label: 'Date', render: v => <span className="text-xs text-muted-foreground hidden lg:block">{formatDate(v)}</span> },
    {
      key: 'id', label: '', render: (_, row) => (
        <div className="flex gap-1" onClick={e => e.stopPropagation()}>
          {canUpdate('expenses') && <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditing(row); setExpenseFormOpen(true); }}><Pencil className="w-4 h-4" /></Button>}
          {canDelete('expenses') && <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeleteId(row.id)}><Trash2 className="w-4 h-4" /></Button>}
        </div>
      )
    }
  ];
  ...
  const typeColumns = [
    ...
    { key: 'created_date', label: 'Created', render: v => <span className="text-sm text-muted-foreground">{formatDate(v)}</span> },
    {
      key: 'id', label: '', render: (_, row) => (
        <div className="flex gap-1" onClick={e => e.stopPropagation()}>
          {canUpdate('expenses') && <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingType(row); setTypeFormOpen(true); }}><Pencil className="w-4 h-4" /></Button>}
          {canDelete('expenses') && <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeleteType(row.id)}><Trash2 className="w-4 h-4" /></Button>}
        </div>
      )
    }
  ];
  ...
  return (
    <div>
      <PageHeader title="Expenses" description="Manage expense types and entries">
        {canCreate('expenses') && <Button variant="outline" size="sm" onClick={() => { setEditingType(null); setTypeFormOpen(true); }} className="gap-1.5"><Tag className="w-4 h-4" /> Create Type</Button>}
        {canCreate('expenses') && <Button onClick={() => { setEditing(null); setExpenseFormOpen(true); }} className="gap-1.5"><Plus className="w-4 h-4" /> Add Expense</Button>}
      </PageHeader>
      ...
      <Tabs defaultValue="expenses" className="space-y-4">
        ...
        <TabsContent value="expenses">
          <DataTable columns={expenseColumns} data={expenses} isLoading={isLoading} searchKey="expense_type_name" expandedContent={expandedContent} />
        </TabsContent>
