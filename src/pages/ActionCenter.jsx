...
import { Target, Plus, AlertTriangle, Flag, Clock, CheckCircle2, TrendingUp } from 'lucide-react';
...
export default function ActionCenter() {
  ...
  const critical  = actions.filter(a => a.priority === 'critical' && !['completed','cancelled'].includes(a.status)).length;
  const high      = actions.filter(a => a.priority === 'high'     && !['completed','cancelled'].includes(a.status)).length;
  const dueToday  = actions.filter(a => a.due_date === today      && !['completed','cancelled'].includes(a.status)).length;
  const overdue   = actions.filter(a => a.due_date && a.due_date < today && !['completed','cancelled'].includes(a.status)).length;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: v => (
        <Badge variant="outline" className={cn('text-xs capitalize', PRIORITY_STYLES[v])}>
          {v}
        </Badge>
      ),
    },
    {
      ...
      render: v => (
        <Badge className={cn('text-xs border-0 capitalize', STATUS_STYLES[v])}>
          {v?.replace('_', ' ')}
        </Badge>
      ),
    },
    {
      ...
      render: (v, row) => {
        ...
        const isOverdue  = v < today && !['completed','cancelled'].includes(row.status);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
        ...
        <SummaryCard title="Critical"      value={formatNumber(critical)}     icon={AlertTriangle} delay={0.05}
          className={critical > 0 ? 'border-red-200 dark:border-red-900' : ''} />
        <SummaryCard title="High Priority" value={formatNumber(high)}         icon={Flag}          delay={0.1} />
        <SummaryCard title="Due Today"     value={formatNumber(dueToday)}     icon={Clock}         delay={0.15}
          className={dueToday > 0 ? 'border-amber-200' : ''} />
        <SummaryCard title="Overdue"       value={formatNumber(overdue)}      icon={TrendingUp}    delay={0.2}
        ...
        {(filterPriority !== 'all' || filterStatus !== 'all' || searchTerm) && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <Button variant="ghost" size="sm" className="h-6 text-xs"
              onClick={() => { setFilterPriority('all'); setFilterStatus('all'); setSearchTerm(''); }}>
              Clear filters
            </Button>
          </div>
        )}
      ...
      <ActionForm
        ...
        editing={editing}
        onSaved={() => qc.invalidateQueries({ queryKey: ['actions'] })}
      />
      <ConfirmDialog
        open={!!deleteId}
        ...
        title="Delete Action?"
        description="This will permanently delete this action."
      />
    </div>
  );
}
